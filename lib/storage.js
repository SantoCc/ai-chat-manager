...
import { parseSearchQuery, conversationMatchesParsedQuery } from './search-query.js';
...
export async function searchConversations({ query = '', platform = '', folderId = '', tag = '' } = {}) {
  ...
  if (query) {
    ...
    // 查询里的平台词与顶部平台 Tab 叠加（取交集）
    if (parsed.platforms.length) {
      conversations = conversations.filter((c) => parsed.platforms.includes(c.platform));
    }
    if (parsed.terms.length || parsed.platforms.length) {
      conversations = conversations.filter((c) => conversationMatchesParsedQuery(c, parsed));
    }
  }
